  ...
  return (
    <div className="relative">
      <div className="flex justify-center items-center text-wrap text-center">
        <h1 className="heading-area text-6xl max-xl:text-5xl ">
          Technical Skills
        </h1>
      </div>
      <div className="mt-4 flex flex-col items-center">
        {techList.map((item, index) => (
          <div key={index} className="flex flex-col items-center mt-6 w-full">
            <h3 className="heading-area tracking-tighter text-3xl text-cyan-500 mb-4">
              {item.category}
            </h3>
            <ul className="flex flex-wrap justify-center gap-4 mt-2 mb-4 w-full text-xl">
              {item.tech.map((tech, techIndex) => (
                <li key={techIndex}>
                  <a
                    className="border-solid border-2 border-cyan-900 rounded-md px-6 py-3 hover:border-cyan-300 text-base flex items-center"
                    href={tech.href}
                    target="_blank"
